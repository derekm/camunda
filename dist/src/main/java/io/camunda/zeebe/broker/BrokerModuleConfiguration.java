...
public class BrokerModuleConfiguration implements CloseableSilently {
  ...
  @Bean
  public ExporterRepository exporterRepository(@Autowired(required = false) List<ExporterFactory> exporterFactories) {
    return new ExporterRepository(exporterFactories);
  }
  ...
  public Broker broker(final ExporterRepository exporterRepository) {
    ...
    broker = new Broker(systemContext, springBrokerBridge, Collections.emptyList(), exporterRepository);
